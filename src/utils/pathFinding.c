...
int checkNextPosition(int y, int x) {
...
int addNeighbors(int** frontier, int frontierCount, int*** cameFrom, int y, int x) {
    // north
    if (y > 0 && cameFrom[y-1][x][0] < 0 && checkNextPosition(y - 1, x)) {
    ...
    // south
    if (y < (MAX_HEIGHT - 1) && cameFrom[y+1][x][0] < 0 && checkNextPosition(y + 1, x)) {
        addPositionYX(frontier, frontierCount, y + 1, x);
        frontierCount++;
        cameFrom[y+1][x][0] = y;
        cameFrom[y+1][x][1] = x;
    ...
    // east
    if (x < (MAX_WIDTH - 1) && cameFrom[y][x+1][0] < 0 && checkNextPosition(y, x + 1)) {
    ...
    // west
    if (x > 0 && cameFrom[y][x-1][0] < 0 && checkNextPosition(y, x - 1)) {
        addPositionYX(frontier, frontierCount, y, x - 1);
        frontierCount++;
        cameFrom[y][x-1][0] = y;
        cameFrom[y][x-1][1] = x;
